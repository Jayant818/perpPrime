...
use crate::{CircularQueue, GlobalConfig, Market, Order, OrderPosition, OrderSide, OrderType, RequestQueue, RequestType, UserAccount, UserPosition, error::ErrorCode, request_queue, user_position};
...
pub struct PlacePerpOrder<'info>{
    ...
    // There is some problem in this as In Request Queue there are 2 struct used Request Queue and Request Header
...
pub fn place_perp_order(ctx:Context<PlacePerpOrder>, amount_in_ui:u64, side:OrderSide,qty_in_ui:u64,pair:String,position:OrderPosition, margin:u64,order_type:OrderType,request_type:RequestType)->Result<()>{
    ...
    let request_queue = ctx.accounts.request_queue.try_borrow_mut_data();
    ...
    let amount_in_lots:u64 = 200;
    let size_in_lots:u64 = 20;
    ...
    if margin < latest_price/size_in_lots {
        // throw error - ErrorCode::InsufficientMargin
    }

    // Check user has sufficient balance or not
    if user_account.collateral_balance < 0 {
        // fund the user account transfer the money.
        let amount_to_be_transferred = amount_in_ui;

        let transfer_struct = TransferChecked{
        ...
        let cpi_context = CpiContext::new(token_program.to_account_info(), transfer_struct);
    ...
    // let user_position = &mut ctx.accounts.
    ...
    user_account.collateral_balance -=amount_in_lots;
    user_account.locked_collateral += amount_in_lots; // can use checked_sum here 
    ...
    let order_id:u128 = (price as u128 << 64 ) | (market.sequence as u64);
    let item = RequestQueue {
        user: user.key(),
        order_id:order_id,
        order_side:side,
    ...
    CircularQueue::push(request_queue, item)?;
